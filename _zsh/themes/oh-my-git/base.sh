## Called twice ?? Here and in prompt.sh

get_current_action () 
{
    local info="$(git rev-parse --git-dir 2>/dev/null)"
    if [ -n "$info" ]; then
		local has_action=true
        local action
